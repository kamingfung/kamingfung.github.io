...
I am lucky enough to have access to NCAR's super computing facilities - Cheyenne allows me to run CESM simulation with hundreds of cores while Casper streamlines my data processing, analysis, and visualization without wasting time transferring large files. But, setting up the correct software to fully deploy these machines is a pain in the neck. :tired_face: Here I would like to share how I set up my `Jupyter` environment to run `R` and `Python` on these two supercomputers.
...
Alternatively, the old-fashioned `IPython` approach to run `Jupyter` can also be found [in the Jupyter and IPython documentation](https://www2.cisl.ucar.edu/resources/jupyter-and-ipython).
